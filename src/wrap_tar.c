...
int	gtar_recover (struct gtar_ccb *gtar);
int	gtar_recover_cmd (struct gtar_ccb *gtar);
...
int
recover_pass_content (struct tar_snoop_cb *tscb, char *data, int n_data_rec)
{
	struct gtar_ccb *	gtar = (struct gtar_ccb *)tscb->app_data;
	int			rc;

	rc = write (gtar->fdmap[0], data, n_data_rec * RECORDSIZE);
	return n_data_rec;
}

int
gtar_recover (struct gtar_ccb *gtar)
{
	int			rc;
	struct tar_snoop_cb 	tscb;
	...
	tscb.notice_file = 0;
	tscb.pass_content = recover_pass_content;

	rc = gtar_recover_cmd (gtar);
	if (rc)
		return rc;

	gtar->fdmap[0] = WRAP_FDMAP_INPUT_PIPE;
	gtar->fdmap[1] = 2; /* WRAP_FDMAP_DEV_NULL;*/
	gtar->fdmap[2] = 2;
	...
	for (;;) {
		...
		rc = tscb_fill (&tscb, gtar->wccb->data_conn_fd);
		if (rc < 0) {
			perror ("tscb_fill()");
			exit(2);
		}
...
int
gtar_recover_cmd (struct gtar_ccb *gtar)
{
	struct wrap_ccb *	wccb = gtar->wccb;
	char *			cmd = gtar->cmd;
	int			i, n_file;
...
ndmda_gtar_prepare_reco_common (struct ndm_session *sess, char *cmd)
{
	...
	pv = ndmda_find_env (sess, "FH_INFO_VALID");
	if (pv) {
